    ...
    def apply(self, customers: List[Customer],
        ...
        for customer in customers:
            if customer.get_id() == int(filter_string):
                for info in data:
                    for num in customer.get_phone_numbers():

                        if info.src_number == num and info.time not in \
                                date_list:
                            filtered_calls.append(info)
                            date_list.append(info.time)

                        elif info.dst_number == num and info.time not in \
                                date_list:
    ...
    def apply(self, customers: List[Customer],
        ...
        for info in data:
            ...
            if filter_string[0] == "L":
                if info.duration < int(filter_string[1:]):
                    calls.append(info)
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        area = []
        for loc in temp:
            area.append(float(loc.replace(" ", "")))
        for info in data:
            src_long = info.src_loc[0]
            src_lat = info.src_loc[1]
            dst_long = info.dst_loc[0]
            dst_lat = info.dst_loc[1]
            if area[0] <= src_long <= area[2]:
                if area[1] <= src_lat <= area[3]:
                    calls.append(info)
            elif area[0] <= dst_long <= area[2]:
                if area[1] <= dst_lat <= area[3]:
                    calls.append(info)
            else:
                return data
        return calls
